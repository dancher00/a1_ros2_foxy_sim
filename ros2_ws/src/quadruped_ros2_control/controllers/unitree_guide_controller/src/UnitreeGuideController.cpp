...
namespace unitree_guide_controller
{
    ...
    controller_interface::return_type UnitreeGuideController::update()
    {
        ...
        // Calculate period if we have a last update time
        rclcpp::Duration period = rclcpp::Duration::from_seconds(0.0);
        if (last_update_time_ != std::chrono::steady_clock::time_point())
        {
            std::chrono::duration<double> time_diff = now - last_update_time_;
            period = rclcpp::Duration::from_seconds(time_diff.count());
        ...
        {
            current_state_->run(current_time, period);
    ...
    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn UnitreeGuideController::on_init()
    {
        try
        {
    ...
    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn UnitreeGuideController::on_configure(
    ...
    {
        // Call the on_init method if needed
        on_init();
        ...
        {
            if (interface.get_name().find(imu_name_) != std::string::npos)
            {
    ...
    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn UnitreeGuideController::on_deactivate(
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        release_interfaces();
        return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS;
