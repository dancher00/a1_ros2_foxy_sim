  ...
  control_input_msgs__msg__Inputs__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    control_input_msgs__msg__Inputs * data =
      (control_input_msgs__msg__Inputs *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!control_input_msgs__msg__Inputs__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          control_input_msgs__msg__Inputs__fini(&data[i]);
        }
        free(data);
        return false;
      }
